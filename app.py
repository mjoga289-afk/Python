...
ADMIN_EMAIL = 'mjoga289@gmail.com'   # change to your email
DB_NAME = 'data.db'
...
def select_random():
    ...
    if admin_email != ADMIN_EMAIL:
        return jsonify({'error': 'Unauthorized'}), 403
    conn = get_db()
    # get eligible participants (joined and not selected before)
...
def set_room():
    ...
    # mark selected participants as room_received
...
def restore():
    ...
    conn = get_db()
    # Disable foreign key checks temporarily for SQLite (optional)
